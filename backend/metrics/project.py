...
async def project_margin(*, org_id: str = ORG_ID, project_id: str = None, **_) -> dict:
    """Margin proyek = `pendapatan diakui - (realisasi RAB + budget operasional realisasi)`.

    Budget operasional belum punya sumber data (fase Target & Budget), jadi margin TIDAK
    dihitung sebagian lalu disebut margin — ia mengaku belum lengkap dan menyebutkan
    komponen yang sudah diketahui, sehingga tetap berguna tanpa menyesatkan.
    """
    rab = await budget_actual(org_id=org_id, project_id=project_id)
    rows = await db.journal_entries.find({"org_id": org_id},
                                         {"_id": 0, "lines": 1}).to_list(20000)
    revenue = 0
    for entry in rows:
        for line in entry.get("lines") or []:
            if line.get("account_type") == "revenue":
                revenue += int(line.get("credit") or 0) - int(line.get("debit") or 0)
    return result("PRJ-07", None, label="Margin proyek", unit="idr",
                  breakdown=[{"key": "revenue", "label": "Pendapatan diakui", "value": revenue},
                             {"key": "rab", "label": "Realisasi RAB (terverifikasi)",
                              "value": rab["inputs"].get("verified", 0)}],
                  inputs={"pendapatan": revenue, "realisasi_rab": rab["inputs"].get("verified", 0)},
                  missing=["budget operasional (non-konstruksi) belum punya sumber data — "
...
METRICS = {
    ...
    "PRJ-07": {"fn": project_margin, "label": "Margin proyek", "unit": "idr",
               "persona": "proyek",
               "formula": "pendapatan diakui - (realisasi RAB + budget operasional)",
               "requires": ["journal_entries", "project_budgets"],
               "drill": "/accounting/reports"},
